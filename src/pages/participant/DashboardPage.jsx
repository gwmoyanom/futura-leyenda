/**
 * pages/participant/DashboardPage.jsx
 *
 * Personal score dashboard. Shows:
 * - Total points + rank
 * - Points breakdown by match
 * - Quick stats (exact scores, correct results, pending)
 */

import { useEffect } from 'react'
import { Link } from 'react-router-dom'
import useStore from '@/store/index.js'
import { Card, CardHeader, CardBody, Spinner, Badge } from '@/components/ui/index.jsx'
...
function StatCard({ value, label, icon, accent = false }) {
  return (
    <div className={clsx(
      ...
      accent ? 'bg-night text-white' : 'bg-white border border-gray-100 shadow-card'
    )}>
      <div className="text-2xl mb-1">{icon}</div>
      <div className={clsx('font-display text-3xl font-bold', accent ? 'text-gold' : 'text-night')}>
        {value}
      </div>
      <div className={clsx('text-xs mt-1', accent ? 'text-white/50' : 'text-gray-400')}>
        {label}
      </div>
    </div>
  )
...
function BreakdownRow({ item }) {
  const { match, prediction, result, points, reason } = item
    ...
    <div className="flex items-center gap-4 py-3 border-b border-gray-100 last:border-0">
      ...
      <div className="flex items-center gap-3 text-xs text-gray-500">
        <div className="text-center">
          ...
          <div className="font-display font-bold text-night">
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Hola, {currentUser?.displayName} {currentUser?.avatar} — así vas en la polla
        </p>
      </div>

      {/* Stats grid */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <StatCard value={totalPoints} label="Puntos totales"   icon="⭐" accent={true} />
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-night">Detalle de partidos</h2>
          ...
          {breakdown.length > 0 ? (
            ...
            <div className="text-center py-12 text-gray-400">
              ...
              <Link to="/predictions" className="mt-4 inline-block">
                <Button size="sm">Ir a predicciones</Button>
              </Link>
            </div>
          )}
        </CardBody>
